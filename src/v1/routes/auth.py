...
async def login_user(user_login: UserCreate, session: AsyncSession = Depends(get_session)):
    ...
    if user is not None:
        ...
        if password_valid:
            access_token = create_access_token(user_data={"email":user.email, "uid":str(user.uid)}, expiry=timedelta(minutes=settings.access_token_expiry))
            refresh_token = create_access_token(user_data={"email":user.email,"uid":str(user.uid)},expiry=timedelta(minutes=settings.refresh_token_expiry),refresh=True)
